...
A simple application using a Calendar to display absences for the default logged in user.  It allows the user to manage their own absences as well
 as see the absences of other users in the project and potential clashes with them.
 
This project uses the sample data provided and extends its display backwards a year as well as forwards a year in order to display a selection 
 of that data without needing to make any modifications to it.
 
This project makes use of the Backpack project open-sourced by Skyscanner because of the advanced options for calendar customisation provided by 
that tooling. For state management this project uses [Redux](http://redux.js.org/docs/introduction/) and associated libraries to take advantage of
their clear delineation of application state and user interactions.
